...
MODEL_DIR = PROJECT_ROOT / "models" / "binary"
...
def load_datasets():
    ...
    with open(MODEL_DIR / "binary_index_to_label.json", "w", encoding="utf-8") as f:
...
def train():
    ...
    print("Binary classes:", class_names)
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            filepath=str(MODEL_DIR / "best_binary_model.keras"),
            monitor="val_accuracy",
            save_best_only=True
        )
    ]

    print("Training frozen binary model...")
    ...
    print("Fine-tuning binary model...")
    ...
    print("Evaluating binary model on test set...")
    test_loss, test_acc = model.evaluate(test_ds)
    print(f"Binary Test Loss: {test_loss:.4f}")
    print(f"Binary Test Accuracy: {test_acc:.4f}")
    ...
    model.save(MODEL_DIR / "final_binary_model.keras")
    print("Saved final binary classifier.")
